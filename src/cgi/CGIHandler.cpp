...
{
    this->_scriptPath = ctx.full_path;
    
    if (ctx.loc && !ctx.loc->cgi_path.empty()) {
        this->_binPath = ctx.loc->cgi_path;
    } else {
        return false; 
    }
    ...
    // 1. 物理检查
    struct stat s;
    if (stat(_scriptPath.c_str(), &s) != 0 || !S_ISREG(s.st_mode)) {
        return false; 
    }
    if (access(_binPath.c_str(), X_OK) != 0) {
        return false; 
    }
