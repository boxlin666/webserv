...
RequestHandler::RequestHandler()
    : _full_path(""), _body_last_modif_date(""), _res_body_len(0), _is_auto_index(false)
{
}

RequestHandler::~RequestHandler(void) {}

void RequestHandler::process_request_handler(const HttpRequest& req, const RouterCtx& route_ctx,
                                             int& status_code)
{
    if (route_ctx.loc) {
...
int RequestHandler::cgi_resource_validator(const RouterCtx& route_ctx)
{
    ...
    struct stat st;
    if (stat(cgi_executable.c_str(), &st) == -1) { return (NOT_FOUND); }
    if (access(cgi_executable.c_str(), X_OK) == 0) return (SUCCESS);
...
int RequestHandler::existing_resource_validator(const RouterCtx& route_ctx)
{
    ...
    if (S_ISDIR(st.st_mode)) return (this->process_directory(route_ctx));
    if (S_ISREG(st.st_mode)) return (this->process_file(st, this->_full_path));
    return (NOT_FOUND);
}
...
int RequestHandler::creatable_resource_validator(void)
{
    ...
    if (stat(this->_full_path.c_str(), &st) == 0) {
        if (access(this->_full_path.c_str(), W_OK) != 0) return (FORBIDDEN);
    }
...
int RequestHandler::extract_parent_path(void)
{
    ...
    if (!_multipart_filename.empty()) this->_full_path += _multipart_filename;
    return (SUCCESS);
}
...
int RequestHandler::process_directory(const RouterCtx& route_ctx)
{
    ...
    for (std::size_t i = 0; i < index.size(); i++) {
        ...
        if (stat(tmp_full_path.c_str(), &st_index) == -1) continue;
        if (!S_ISREG(st_index.st_mode)) continue;
...
{
    if (access(input_path.c_str(), R_OK) == -1) return (FORBIDDEN);
