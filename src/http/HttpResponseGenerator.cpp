...
void HttpResponse::_build_status_line(const HttpRequest& request)
{
    ...
    if (it != this->_status_msg_map.end())
        this->_status_line = request.get_version() + " " + str_status_code + " " +
                             this->_status_msg_map[this->_status_code] + "\r\n";
    else
        this->_status_line =
            request.get_version() + " " + str_status_code + " " + "Internal Server Error" + "\r\n";
}
...
std::string HttpResponse::_generate_content_type(void) const
{
    ...
    Utils::to_lowercase(ext);
    it = this->_ext_map.find(ext);
    if (it != this->_ext_map.end()) return (it->second);
