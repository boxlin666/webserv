...
void ServerConfig::_handle_root(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    if (access(path.c_str(), R_OK | X_OK) != 0)
        throw std::runtime_error(
            "Configuration error: 'root' path \"" + path +
            "\" exists, but server process has NO permission to access it (Permission denied)!");

    if (loc == NULL) {
        if (!this->_root.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'root' path found in server block");
        this->_root = path;
    } else {
        if (!loc->root.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'root' path found in location block");
        loc->root = path;
    }
...
void ServerConfig::_handle_server_name(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    while (++pos < tokens.size() && tokens[pos].content != ";" &&
           tokens[pos].line == current_line) {
        if (!_is_valid_server_name(tokens[pos].content))
            throw std::runtime_error("Syntax error: server name value '" + tokens[pos].content +
                                     "' is invalid");
    ...
    if (this->_server_names.size() == 0)
        throw std::runtime_error(
            "Syntax error: Missing server_name value after server_name directive");
    Utils::expect_semicolon(tokens, pos);
}
...
bool ServerConfig::_is_valid_server_name(const std::string& server_name) const
{
    ...
    for (std::size_t i = 0; i < server_name.length(); ++i) {
        ...
        if (std::iscntrl(c) || c <= 32 || c == 127) return (false);
    ...
    while (pos < tokens.size() && std::isdigit(tokens[pos].content[0])) {
        if (tokens[pos].content.length() == 3 && Utils::is_digit_str(tokens[pos].content)) {
            ...
            if (error_code >= 400 && error_code <= 599)
                ...
                throw std::runtime_error("Syntax error: Invalid status code " +
                                         tokens[pos].content + " for 'error_page'");
        } else
            throw std::runtime_error("Syntax error: Invalid status code " + tokens[pos].content +
                                     " for 'error_page'");
        pos++;
    }
                                                ...
                                                location* loc)
{
    ...
    if (std::isalpha(last_char) && multiplier >= 1024) num_val = val.substr(0, val.length() - 1);
    ...
    if (!(ss >> parsed_num) || !ss.eof())
        throw std::runtime_error("Configuration error: 'client_max_body_size' value '" + val +
                                 "' is overflowed");
...
void ServerConfig::_handle_methods(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    while (++pos < tokens.size() && tokens[pos].content != ";" &&
        ...
        if (tokens[pos].content != "GET" && tokens[pos].content != "POST" &&
            ...
            throw std::runtime_error("Configuration error: method " + tokens[pos].content +
                                     " is not allowed.");
        if (loc == NULL) {
...
void ServerConfig::_handle_return(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    if (url.empty()) throw std::runtime_error("Return URL cannot be empty");
    ...
    bool is_absolute = (url.compare(0, 7, "http://") == 0 || url.compare(0, 8, "https://") == 0);
    bool is_relative = (url[0] == '/');

    if (!is_absolute && !is_relative)
        throw std::runtime_error(
            "Invalid return URL format: must start with 'http://', 'https://' or '/'");
...
void ServerConfig::_handle_upload_path(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    if (access(path.c_str(), R_OK | W_OK | X_OK) != 0)
        throw std::runtime_error(
            "Configuration error: 'upload_path' \"" + path +
            "\" exists, but server process has NO permission to access it (Permission denied)!");

    if (loc == NULL) {
        if (!this->_upload_path.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'upload path' found in server block");
        this->_upload_path = tokens[pos].content;
    } else {
        if (!loc->upload_path.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'upload path' found in location block");
...
void ServerConfig::_handle_cgi_path(std::vector<Token>& tokens, size_t& pos, location* loc)
{
    ...
    if (stat(path.c_str(), &info) != 0)
        ...
        throw std::runtime_error("Configuration error: 'cgi_path' \"" + path +
                                 "\" is a directory, but an executable file is required!");

    if (access(path.c_str(), X_OK) != 0)
        throw std::runtime_error(
            "Configuration error: 'cgi_path' \"" + path +
            "\" exists, but server process has NO executable permission (X_OK Denied)!");

    if (loc == NULL) {
        if (!this->_cgi_path.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'cgi_path' found in server block");
        this->_cgi_path = tokens[pos].content;
    } else {
        if (!loc->cgi_path.empty())
            throw std::runtime_error(
                "Configuration error: Multiple 'cgi_path' found in location block");
...
void ServerConfig::_fill_location_defaults(location& loc)
{
    ...
    if (loc.index.empty()) {
        if (!this->_index.empty()) loc.index = this->_index;
    }
    ...
    if (loc.upload_path.empty()) loc.upload_path = this->_upload_path;
...
void ServerConfig::_validate_location(const location& loc) const
{
    ...
    if (loc._prefix[0] != '/')
        throw std::runtime_error("Config Error: location prefix \"" + loc._prefix +
                                 "\" must start with '/'!");

    // KO if //uploads
    if (loc._prefix.find("//") != std::string::npos)
        throw std::runtime_error(
            "Config Error: consecutive slashes '//' are forbidden in location prefix \"" +
            loc._prefix + "\"!");
    ...
    if ((loc.cgi_path.empty() && !loc.cgi_ext.empty()) ||
        ...
        throw std::runtime_error(
            "Config Error: missing cgi_path or cgi_ext for location prefix \"" + loc._prefix +
            "\"");
    }
    ...
    if (!loc.cgi_path.empty() && !loc.cgi_ext.empty()) {
        if (loc.cgi_ext == ".py" && loc.cgi_path.find("python") == std::string::npos)
            throw std::runtime_error(
                "Config Error: .py extension must match a python CGI path for prefix \"" +
                loc._prefix + "\"");

        if (loc.cgi_ext == ".php" && loc.cgi_path.find("php") == std::string::npos)
            throw std::runtime_error(
                "Config Error: .php extension must match a php CGI path for prefix \"" +
                loc._prefix + "\"");
    }
}
...
void ServerConfig::print() const
{
    ...
    for (size_t i = 0; i < this->_listen_addrs.size(); ++i) {
        std::cout << i << ": Host = " << this->_listen_addrs[i].first
                  << "; Port number = " << this->_listen_addrs[i].second << std::endl;
    }
...
const std::string& ServerConfig::get_error_page(int status_code) const
{
    ...
    if (it != _error_pages.end()) { return it->second; }
